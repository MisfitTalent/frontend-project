...
const createMessageId = () => `client-message-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`;
const isClientMessage = (note: INoteItem) => note.kind === "client_message" ||
...
export const ClientMessageCenter = ({ compact = false, }: ClientMessageCenterProps) => {
    ...
    const handleSubmit = async (values: MessageFormValues) => {
        const representative = representativeOptions.find((option) => option.id === values.representativeId);
    ...
    return (<Card className={styles.card}>
      ...
      <div className={styles.container}>
        <div className={styles.header}>
          <div className={styles.headerCopy}>
            ...
            Send account questions and commercial follow-ups to the representative supporting your workspace.
            </Typography.Text>
          </div>
        ...
        {clientMessages.length === 0 ? (<Empty description="No messages have been sent from this client workspace yet." image={Empty.PRESENTED_IMAGE_SIMPLE}/>) : (<div className={styles.list}>
            {clientMessages.slice(0, compact ? 3 : 6).map((note) => (<div className={styles.messageCard} key={note.id}>
                <div className={styles.messageHeader}>
                  <div className={styles.metaGroup}>
                    ...
                    <Space size="small" wrap>
                      ...
                      <Tag color={getMessageStatusColor(note.status)}>{note.status ?? "Sent"}</Tag>
                      <Tag color="default">{note.createdDate}</Tag>
                    </Space>
                  </div>
                  <Button icon={<SendOutlined />} onClick={() => openComposer(representativeOptions.find((option) => option.id === note.representativeId ||
                option.name === getRepresentativeName(note))?.id)}>
                    Follow up
                  </Button>
                </div>
      ...
      <Modal forceRender onCancel={closeComposer} onOk={() => form.submit()} okButtonProps={{ loading: isSubmitting }} okText="Send message" open={isModalOpen} title="Message a representative">
        <Form className={styles.modalForm} form={form} layout="vertical" onFinish={handleSubmit}>
          <Form.Item label="Representative" name="representativeId" rules={[{ message: "Please choose a representative", required: true }]}>
            <Select options={representativeOptions.map((option) => ({
            label: option.label,
            value: option.id,
          ...
          <Form.Item label="Message" name="content" rules={[{ message: "Please enter your message", required: true }]}>
